...
qtdemux_callback2(GstElement * element, GstPad * new_pad, gpointer app)
{
    ...
    if (strcmp(GST_PAD_NAME(new_pad), "audio_0") == 0)
    {
        ...
        GstPad * audio_bin_sink = gst_element_get_static_pad(my->audio_bin, "sink");
    ...
    else
    {
        ...
        GstPad * video_bin_sink = gst_element_get_static_pad(my->video_bin, "sink");
...
int make_rtp_bins()
{
    ...
    myrtp->pipeline = GST_PIPELINE(gst_pipeline_new(NULL));
    ...
    myrtp->rtpbin = gst_element_factory_make("rtpbin", "myrtpbin");
    g_object_set(myrtp->rtpbin, "rtp-profile", GST_RTP_PROFILE_AVPF, NULL);

    gst_bin_add(GST_BIN(myrtp->pipeline), myrtp->rtpbin);
    ...
    gst_bin_add_many(GST_BIN(myrtp->pipeline), 
                     ...
                     NULL);
    g_object_set(myrtp->rtpsink_video, "port", 5000, "host", "192.168.0.4", NULL);
    g_object_set(myrtp->rtcpsink_video, "port", 5001, "host", "192.168.0.4", NULL, "sync", FALSE, "async", FALSE, NULL);
    g_object_set(myrtp->rtcpsrc_video, "port", 5002, NULL);

    
    pad_name = g_strdup_printf("send_rtp_sink_%u", 0);
    gst_element_link_pads(GST_ELEMENT(myrtp->video_bin), "src", GST_ELEMENT(myrtp->rtpbin), pad_name);
    g_free(pad_name);

    pad_name = g_strdup_printf("send_rtp_src_%u", 0);
    ...
    pad_name = g_strdup_printf("send_rtcp_src_%u", 0);
    gst_element_link_pads(GST_ELEMENT(myrtp->rtpbin), pad_name, myrtp->rtcpsink_video, "sink");
    g_free(pad_name);

    pad_name = g_strdup_printf("recv_rtcp_sink_%u", 0);
    ...
    gst_pad_set_active(ghost_bin_sink_pad_audio, myrtp->audio_bin);
    ...
    g_object_set(myrtp->rtcpsink_audio, "port", 5005, "host", "192.168.0.4", NULL, "sync", FALSE, "async", FALSE, NULL);
    g_object_set(myrtp->rtcpsrc_audio, "port", 5006, NULL);

    pad_name = g_strdup_printf("send_rtp_sink_%u", 1);
    gst_element_link_pads(GST_ELEMENT(myrtp->audio_bin), "src", GST_ELEMENT(myrtp->rtpbin), pad_name);
    g_free(pad_name);

    pad_name = g_strdup_printf("send_rtp_src_%u", 1);
    ...
    pad_name = g_strdup_printf("recv_rtcp_sink_%u", 1);
    ...
    g_object_set(myrtp->file_src, "location", "/home/ysh8361/SampleVideo_1280x720_5mb.mp4", NULL);
