...
static void
qtdemux_pad_added_cb3 (GstElement * element, GstPad * new_pad, gpointer app)
{
   CallData *call = (CallData*)app;
    GstPad *sinkpad, *srcpad;
 ...
 */
static void
add_stream (GstPipeline * pipe, GstElement * rtpBin, SessionData * session, const char *szIp, int nPort)
{
  ...
  g_object_set (rtpSink, "port", nPort, "host", szIp, NULL);
  g_object_set (rtcpSink, "port", nPort + 1, "host", szIp, "sync",
      FALSE, "async", FALSE, NULL);
  g_object_set (rtcpSrc, "port", nPort + 5, NULL);
...
main (int argc, char **argv)
{
    ...
    for (index = 0; index < 300; index++)
    {
        ...
        {
            g_print("i : %d\n", index);
            add_stream(pipe, rtpBin, videoSession, "192.168.0.204", 5000);
            add_stream(pipe, rtpBin, audioSession, "192.168.0.204", 5006);
        }
        else
        {
            g_print("i : %d, basePort:%d\n", index, basePort);
            add_stream(pipe, rtpBin, videoSession, "192.168.0.192", basePort);
            add_stream(pipe, rtpBin, audioSession, "192.168.0.192", basePort + 6);
        }

        basePort = basePort + 12;
        GstElement *mp4Src = gst_element_factory_make("filesrc", NULL);
        GstElement *demux = gst_element_factory_make("qtdemux", NULL);

        g_object_set(mp4Src, "location", "/home/jdin/SampleVideo_1280x720_5mb.mp4", NULL);
        gst_bin_add_many(GST_BIN(pipe), mp4Src, demux, NULL);
        gst_element_link(mp4Src, demux);
        g_signal_connect(demux, "pad-added", G_CALLBACK(qtdemux_pad_added_cb3), call);
