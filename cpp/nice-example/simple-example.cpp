...
main(int argc, char *argv[])
{
  ...
  // g_object_set(agent, "upnp-timeout", upnp_timeout, NULL);
    ...
    gpointer data)
{
  ...
  if (g_io_channel_read_line (source, &line, NULL, NULL, NULL) ==
    ...
    if (rval == EXIT_SUCCESS) {
      ...
      ret = FALSE;
      g_print("waiting for state READY or FAILED signal...");
    } else {
    ...
    gpointer data)
{
  ...
  if (state == NICE_COMPONENT_STATE_CONNECTED) {
    ...
    ret = nice_agent_get_selected_pair (agent, _stream_id, component_id, &local, &remote);
    g_print("ret:%d\n", ret);
    if (ret) {
...
cb_new_selected_pair(NiceAgent *agent, guint _stream_id,
    ...
    gchar *rfoundation, gpointer data)
{
  g_print("SIGNAL: selected pair %s %s", lfoundation, rfoundation);
}
    ...
    guint component_id, char *line)
{
  ...
  printf("nice_agent_set_remote_credentials.. _stream_id:%d, ufrag:%s, passwd:%s\n", 
    _stream_id, 
    ufrag, 
    passwd);
  ...
  ret = nice_agent_set_remote_candidates(agent, _stream_id, component_id,
      remote_candidates);
  g_print("nice_agent_set_remote_candidates ret:%d\n", ret);
  if (ret < 1) {
  ...
  // local_candidates = nice_agent_get_remote_candidates(agent, _stream_id, component_id);
  // g_print("nice_agent_get_local_candidates start..%p\n", local_candidates);
  // for(auto item = local_candidates; item != NULL; item = item->next){
  //   g_print("nice_agent_get_local_candidates..\n");
  // }
