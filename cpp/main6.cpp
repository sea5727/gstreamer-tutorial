...
qtdemux_pad_added_cb (GstElement * element, GstPad * new_pad, gpointer app)
{
    ...
    {
        g_print("add pads..: %s\n", GST_PAD_NAME(new_pad));
        sinkpad = gst_element_get_static_pad(GST_ELEMENT(audioBin), "sink");
        g_assert(sinkpad);
    ...
    {
        g_print("add pads..: %s\n", GST_PAD_NAME(new_pad));
        sinkpad = gst_element_get_static_pad(GST_ELEMENT(videoBin), "sink");
        g_assert(sinkpad);
...
static SessionData *
make_audio_session2 (guint sessionNum)
{
  SessionData *session;
  audioBin = GST_BIN (gst_bin_new (NULL));
  ...
  gst_bin_add_many (audioBin, audio_queue, audio_rtppay, NULL);

  // gst_element_link_many (audioSrc, demux, NULL);
  gst_element_link_many (audio_queue, audio_rtppay, NULL);

  // g_signal_connect (demux, "pad-added", G_CALLBACK (qtdemux_pad_added_cb), audio_queue);

  setup_ghost (audio_rtppay, audioBin);
  GstPad* pad = gst_element_get_static_pad (audio_queue, "sink");
  GstPad* ghost_pad = gst_ghost_pad_new("sink", pad);
  gst_pad_set_active (ghost_pad, TRUE);
  gst_element_add_pad (GST_ELEMENT(audioBin), ghost_pad);
...
{
  videoBin = GST_BIN (gst_bin_new (NULL));
  ...
  gst_bin_add_many (videoBin, video_queue, video_parse, video_rtppay, NULL);

  // g_signal_connect (demux, "pad-added", G_CALLBACK (qtdemux_pad_added_cb2), video_queue);
  
  // gst_element_link (videoSrc, demux);
  gst_element_link_many (video_queue, video_parse, video_rtppay, NULL);

  setup_ghost (video_rtppay, videoBin);
  
  GstPad* pad = gst_element_get_static_pad (video_queue, "sink");
  GstPad* ghost_pad = gst_ghost_pad_new("sink", pad);
  gst_pad_set_active (ghost_pad, TRUE);
  gst_element_add_pad (GST_ELEMENT(videoBin), ghost_pad);
  gst_object_unref (pad);
...
main (int argc, char **argv)
{
  ...
  audioSession = make_audio_session2 (1);
  ...
  gst_element_link (videoSrc, demux);
  g_signal_connect (demux, "pad-added", G_CALLBACK (qtdemux_pad_added_cb), NULL);
