...
gst_my_plugin_class_init(GstMyPluginClass * klass) {
    ...
    g_object_class_install_properties (gobject_class, PROP_SILENT, silent);
...
gst_my_plugin_plugin_init (GstPlugin * plugin) {
    GST_DEBUG_CATEGORY_INIT (gst_my_plugin_debug, "my_plugin", 0, "my Plugin!!");

    return gst_element_register(plugin, "my_plugin", GST_RANK_NONE, GST_TYPE_MY_PLUGIN);
}
