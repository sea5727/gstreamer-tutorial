...
make_pipeline(char * name, char *host, int port)
{
    ...
    g_object_set(newdata->file_src, "location", "/home/ysh8361/youtube_ZwTWatG3fL8_1080p_mhd.mp4", NULL);
...
int make_rtp_bins(const char *ip, int port, int video_seqq, int audio_seq, const char *file_path)
{
    ...
    g_object_set(myrtp->file_src, "location", file_path, NULL);
...
int main(int argc, char *argv[])
{
    if(argc < 6)
    {
        g_print("input <your_ip> <video_port> <max_count> <my_index> <filepath>\n");
        return -1;
    }
    ...
    for(int i = 0 ; i < count ; i++)
    {
        if(i == myidx)
        {
            make_rtp_bins(argv[1], port, seq, seq+1, argv[5]); seq += 2;
        } 
        else 
        {
            make_rtp_bins("192.168.0.192", garbage_port, seq, seq+1, argv[5]); seq += 2;
            garbage_port += 8;
        }
    }
